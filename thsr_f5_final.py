...
def preprocess_input(job_features, candidate_features, label_encoders):
    ...
    """
    processed_features = {}
    # Combinei recursos de trabalho e candidato em um único dicionário para facilitar o processamento
    all_features = {**job_features, **candidate_features}

    for col, le in label_encoders.items():
        value = all_features.get(col, '') # Get the value, default to empty string if not found
        try:
           # Use o codificador de rótulo carregado para transformar o valor de entrada
# Manipule valores não vistos atribuindo um valor padrão (por exemplo, 0 ou uma categoria dedicada 'não vista')
# Aqui, usaremos 0, supondo que o codificador foi instalado em um conjunto abrangente
            processed_features[col + '_enc'] = le.transform([str(value).lower().strip()])[0]
        except ValueError:

# Lida com casos em que a transformação falha (por exemplo, rótulo não visto)
...
if st.button("Get Recommendation"):
    # Combine recursos de trabalho e candidato para pré-processamento
    input_features = {**job_features, **candidate_features}
...
# Fazer previsão
# Garantir que o número de recursos corresponda à expectativa do modelo
# Se o modelo foi treinado apenas em recursos estruturados, use processed_input diretamente
# Se o modelo foi treinado em recursos estruturados + texto, precisaríamos processar texto aqui também
# Com base no notebook anterior, o modelo foi treinado apenas em recursos estruturados.
    if processed_input.shape[1] == model.n_features_in_:
        probability = model.predict_proba(processed_input)[:, 1]
        st.subheader("Recommendation:")
...
# Usando 0,5 como um limite simples
            st.success("Recommended: This candidate has a high probability of being hired for this job.")
        else:
            st.info("Not Recommended: This candidate has a lower probability of being hired for this job.")
    else:
...
# Carregue o modelo salvo e os codificadores de rótulo
try:
    model = joblib.load('random_forest_model.joblib')
    label_encoders = joblib.load('label_encoders.joblib')
except FileNotFoundError:
...
# Combinei características de emprego e candidato em um único dicionário para facilitar o processamento
    all_features = {**job_features, **candidate_features}

    for col, le in label_encoders.items():
        value = all_features.get(col, '') # Get the value, default to empty string if not found
        try:
          # Usei o codificador de rótulo carregado para transformar o valor de entrada
# Manipule valores não vistos atribuindo um valor padrão (por exemplo, 0 ou uma categoria dedicada 'não vista')
# Aqui, usaremos 0, supondo que o codificador foi instalado em um conjunto abrangente
            processed_features[col + '_enc'] = le.transform([str(value).lower().strip()])[0]
        except ValueError:

# Lidar com casos em que a transformação falha (por exemplo, rótulo não visto)
            processed_features[col + '_enc'] = 0 # Assign a default value
...
# Combinei recursos de trabalho e candidato para pré-processamento
    input_features = {**job_features, **candidate_features}
# Pré-processado os dados de entrada
    processed_input = preprocess_input(job_features, candidate_features, label_encoders)

# Fazer previsão
# Garanti que o número de recursos corresponda à expectativa do modelo
# Se o modelo foi treinado apenas em recursos estruturados, use processed_input diretamente
# Se o modelo foi treinado em recursos estruturados + texto, precisaríamos processar texto aqui também
# Com base no notebook anterior, o modelo foi treinado apenas em recursos estruturados.
    if processed_input.shape[1] == model.n_features_in_:
...
## Salvar codificadores de modelo e rótulo


Subtarefa:
Salvar o modelo Random Forest treinado e os LabelEncoders ajustados em arquivos usando joblib..
...
"""## Criar script para o aplicativo Streamlit

### Subtarefa:
Escrever o script Python para o aplicativo Streamlit que carregará o modelo e os codificadores salvos, criará uma interface de usuário para a entrada, pré-processará a entrada, fará uma previsão usando o modelo carregado e exibirá a recomendação.

**Raciocínio**:
Criar o script do aplicativo Streamlit, incluindo o carregamento do modelo e dos codificadores, a definição da função de pré-processamento, a configuração da interface do usuário e a realização de previsões.
"""
...
# Carreguei o modelo salvo e os codificadores de rótulo
try:
    model = joblib.load('random_forest_model.joblib')
    label_encoders = joblib.load('label_encoders.joblib')
except FileNotFoundError:
...
# Combinei características de emprego e candidato em um único dicionário para facilitar o processamento
    ...
    for col, le in label_encoders.items():
        value = all_features.get(col, '') # Obter o valor, padrão para string vazia se não for encontrado
        try:
          # Use o codificador de rótulo carregado para transformar o valor de entrada
# Manipule valores não vistos atribuindo um valor padrão (por exemplo, 0 ou uma categoria dedicada 'não vista')
# Aqui, usaremos 0, supondo que o codificador foi instalado em um conjunto abrangente
...
# Adicionar string vazia para seleção
        job_features[col] = st.selectbox(f"Job {col.replace('vaga_', '').replace('_', ' ').title()}:", options)
...
"""
**Razão**:
O comando anterior falhou porque a biblioteca `streamlit` não está instalada. Instale `streamlit` usando pip.
...
## Teste o aplicativo streamlit


### Subtarefa:
Testar o aplicativo implantado com entradas de amostra para garantir que esteja funcionando corretamente e fornecer recomendações.
"""